...
class ParseContext(object):
  ...
  def parse_add_test(self, tokens, _breakstack):
    ...
    if tokens[0].type not in (
        lexer.TokenType.WORD, lexer.TokenType.UNQUOTED_LITERAL):
    ...
    while tokens and tokens[0].type != lexer.TokenType.RIGHT_PAREN:
      token = tokens.pop(0)
      if token.type is lexer.TokenType.WHITESPACE:
        continue
      if token.type is lexer.TokenType.QUOTED_LITERAL:
  ...
  def parse_set_tests_properties(self, tokens, _breakstack):
    ...
    while (tokens
           ...
           and tokens[0].type != lexer.TokenType.RIGHT_PAREN):
      token = tokens.pop(0)
