...
function(format_and_lint module)
  ...
  # TODO(josh): I'm pretty sure this can be covered by a cmake_parse_args(args
  # "" "" "CMAKE;CC;CCDEPENDS;PY" ${ARGN}) where the "auto" list is just
  # ${args_UNPARED_ARGUMENTS}
  foreach(arg ${ARGN})
    # assign by filename
  ...
  if(cmake_files_)
    ...
    # cmake-format: on
    list(APPEND depfiles_ ${cmake_files_} ${CMAKE_SOURCE_DIR}/.cmake-format.py)
  endif()
  if(cc_files_)
    # cmake-format: off
    list(
      APPEND fmtcmds_
      COMMAND clang-format-6.0 -style file -i ${cc_files_})
    # cmake-format: on
    list(APPEND lntcmds_ COMMAND clang-tidy-6.0 -p ${CMAKE_BINARY_DIR}
                ${cc_files_})
    list(APPEND lntcmds_ COMMAND cpplint ${cc_files_})
    list(APPEND depfiles_ ${cc_files_} ${CMAKE_SOURCE_DIR}/.clang-format
  ...
  if(py_files_)
    ...
    # note entirely within the package tree from the root of the repository. As
    # such pylint will not match the rcfile in the root of the repository.
    # cmake-format: off
    list(
      APPEND lntcmds_
      COMMAND env PYTHONPATH=${CMAKE_SOURCE_DIR}
    ...
    list(
      APPEND lntcmds_
      COMMAND env PYTHONPATH=${CMAKE_SOURCE_DIR} flake8 --jobs 1 ${py_files_})
    # cmake-format: on
    list(APPEND depfiles_ ${py_files_} ${CMAKE_SOURCE_DIR}/.flake8
                ${CMAKE_SOURCE_DIR}/.pep8 ${CMAKE_SOURCE_DIR}/pylintrc)
  endif()
  if(js_files_)
    # list(APPEND fmtcmds_ COMMAND esformat ${js_files_})
    list(APPEND lntcmds_ COMMAND eslint ${js_files_})
    list(APPEND depfiles_ ${js_files_} ${CMAKE_SOURCE_DIR}/.eslintrc.js)
  endif()

  set(fmtstamp_ ${CMAKE_CURRENT_BINARY_DIR}/${module}_format.stamp)
  add_custom_command(
    OUTPUT ${fmtstamp_} ${fmtcmds_}
